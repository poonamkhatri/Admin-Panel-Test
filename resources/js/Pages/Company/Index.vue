	...
	<AppLayout title="Company">
		...
		<template #header>
			<h2 class="flex justify-between text-xl font-semibold leading-tight text-gray-800">
				...
				<inertia-link href="companies/create">
					<a
		...
		<div class="py-12">
			...
			<div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
				<div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
					...
					<a href="#" @click="deleteSelectedcompanies"
						...
						<tbody class="bg-white divide-y divide-gray-200">
							<tr v-for="company in companies.data" :key="company.id">
										...
										<div class="ml-4">
											<div class="text-sm font-medium text-gray-900">
												<inertia-link class="transition hover:text-blue-500"
													:href="`companies/${company.id}`">{{ company.name }}</inertia-link>
											</div>
										</div>
									</div>
								</td>
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex items-center justify-center">
										...
										<div class="ml-4">
											<div class="text-sm font-medium text-gray-900">
												<inertia-link class="transition hover:text-blue-500"
													:href="`companies/${company.id}`">{{ company.website }}</inertia-link>
											</div>
										</div>
									</div>
								</td>
								...
								<td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
									...
									<inertia-link :href="`/companies/${company.id}/edit`"
...
export default {
	...
	methods: {
		...
		deletecompany: function (company) {
			this.confirmAction('Are you sure you want to delete company?', function () {
				this.$inertia.delete(route('companies.destroy', company.id));
			}.bind(this));
		},
		search() {
			this.$inertia.replace(this.route('companies.index', { term: this.term }))
		},
		async deleteSelectedcompanies() {
			...
			const confirmed = confirm(`Are you sure you want to delete ${this.selectedcompanies.length} company(s)?`);
			if (!confirmed) {
				return;
			}

			try {
				await this.$inertia.post('/companies/delete', { ids: this.selectedcompanies });
				this.selectedcompanies = [];
			} catch (error) {
				console.error(error);
				alert('An error occurred while deleting the selected companies.');
			}
		},
		computed: {
			companyAvatar(company) {
				return this.company.avatar !== 'placeholder'
					? this.company.avatar
